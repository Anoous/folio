...
对每个关键词做 pg_trgm `%` 操作符匹配（走 GIN 索引），取 union 去重。

```sql
-- 事务级设置低阈值，宽松召回
SELECT set_config('pg_trgm.similarity_threshold', '0.05', true);

WITH keyword_matches AS (
    SELECT DISTINCT ON (a.id)
        a.id, a.title, a.summary, a.key_points, a.site_name, a.created_at,
        GREATEST(
            similarity(a.title, kw.word),
            COALESCE(similarity(a.summary, kw.word), 0)
        ) AS score
    FROM articles a
    ...
    WHERE a.user_id = $1
      ...
      AND (a.title % kw.word OR a.summary % kw.word)
...
- **`%` 操作符**走 GIN 索引，比 `similarity() > threshold` 快数量级。
- **事务级 `set_config`** 避免影响连接池中其他查询。
- `DISTINCT ON (id)` 确保同篇文章只出现一次，取最高分。
- 返回 `key_points` 给下游 LLM 更多上下文。
- **预期性能**：5000 篇 × 15 关键词，GIN 索引下 < 100ms。
...
```
8. semantic_keywords: 生成 10-15 个语义关键词，用于后续检索匹配。
   要求：包含核心概念的中英文双语表达、同义词、上下位概念。
   不要与 tags 重复。目的是让未来的关键词搜索能找到这篇文章。
```
...
3. 将 30 篇候选摘要 + 本文摘要送 DeepSeek：
   - prompt: "本文是《{title}》: {summary}。以下是候选文章，选出最相关的 5 篇，说明关联理由。"
4. 结果写入 `article_relations` 表
...
| Domain | `domain/rag.go` | 修改 | `RAGSource` 加 `KeyPoints []string` |
| Client | `client/ai.go` | 修改 | `AnalyzeResponse` 加 `SemanticKeywords`；新增 `ExpandQuery`、`RerankArticles` |
| Client | `client/ai_mock.go` | 修改 | 三个方法的 mock 实现 |
| Repository | `repository/article.go` | 修改 | `UpdateAIResult` 写入 semantic_keywords；新增 `BroadRecall`、`BroadRecallFull` |
| Repository | 新文件 `repository/relation.go` | 新增 | article_relations CRUD |
| Service | `service/rag.go` | 修改 | `applyTokenBudget` 替换 ≥500 篇逻辑 |
| Service | `service/article.go` | 修改 | 新增 `SemanticSearch` 方法 |
| Handler | `handler/search.go` | 修改 | 读取 `mode` 参数，分流 |
| Handler | 新文件 `handler/relation.go` | 新增 | `GET /articles/{id}/related` |
| Router | `api/router.go` | 修改 | 注册 related 端点 |
| Worker | `worker/ai_handler.go` | 修改 | AI 完成后入队 `article:relate` |
