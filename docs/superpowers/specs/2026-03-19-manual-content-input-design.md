...
DROP INDEX articles_user_id_url_idx;
...
DROP INDEX articles_user_id_url_idx;
CREATE UNIQUE INDEX articles_user_id_url_idx ON articles (user_id, url);
...
- **Repository 影响**：所有 `article.go` 中的 Scan 调用（`GetByID`、`ListByUser`、`Search` 等）需将 `&a.URL` 改为 `sql.NullString` 扫描再转换为 `*string`，或使用 pgx 的 nullable 支持
...
| Go Repository | 所有 article Scan 调用适配 nullable URL |
