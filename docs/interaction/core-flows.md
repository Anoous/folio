...
采用底部 Tab Bar，共 3 个 Tab。克制到极致——不是 4 个，不是 5 个，只有 3 个。
...
**为什么是 Tab Bar 而不是无 Tab？**

Folio 不是时光印记那种只有一个核心体验的产品。Folio 是工具，工具需要清晰的导航。用户要在「浏览」和「搜索」之间频繁切换，Tab Bar 是最高效的方式。但只给 3 个 Tab——因为多了就不克制了。
