...
增值功能（远期）
├── F10. 每日播客   — 基于当日收藏生成语音日报 + 文本摘要
├── F11. 知识关联   — 自动发现文章间的隐含联系，构建知识网络
├── F12. 跨文章综合 — 基于多篇收藏综合回答问题
├── F13. 智能专题   — 自动围绕主题聚合文章，生成专题概览
├── F14. 知识唤醒   — 基于遗忘曲线定期浮现关键洞察
└── F15. 时效预警   — 检测收藏内容是否可能已过时
```
...
| F10. 每日播客 | P3 | 否 | 基于当日收藏生成语音日报 + 文本摘要 |
| F11. 知识关联 | P2 | 否 | 自动发现文章间的隐含联系，阅读页展示相关收藏 |
| F12. 跨文章综合 | P2 | 否 | 基于多篇收藏综合回答用户问题，标注来源 |
| F13. 智能专题 | P2 | 否 | 围绕主题自动聚合文章，生成专题概览 |
| F14. 知识唤醒 | P3 | 否 | 基于遗忘曲线定期浮现关键洞察 |
| F15. 时效预警 | P3 | 否 | 检测收藏内容是否可能已过时 |
...
**用户感受**：每天早上打开 Folio，一份基于昨天收藏内容的语音日报已经准备好了。戴上耳机，边通勤边听昨天的知识精华；没时间听也没关系，文本版摘要随时可看。

| 项目 | 要求 |
|------|------|
| 生成时机 | 每日固定时间或用户手动触发 |
| 触发条件 | 前一日至少收藏 1 篇文章 |
| 语音时长 | 3-10 分钟（根据文章数量自动调节） |
| 语音风格 | 简洁、信息密度高的播报风格，不是简单拼接摘要，而是有逻辑的叙述 |
| 文本版 | 按主题分组的结构化日报，300-800 字 |
| 语言 | 跟随文章主体语言 |
| 播放器 | 暂停/继续、进度拖拽、倍速（0.75x-2x） |

---

### F11. 知识关联（远期 · P2 · Pro+）

**用户感受**：读一篇关于向量数据库的文章，底部自动出现"你 2 周前收藏的《RAG 实践指南》与此高度相关"——原来这些知识是连在一起的。

Folio 自动发现收藏文章之间的隐含关联，把平铺的收藏库变成一张有连接的知识网络。

| 项目 | 要求 |
|------|------|
| 展示位置 | 阅读页底部「相关收藏」区域 |
| 关联数量 | 每篇展示 3-5 篇最相关的收藏 |
| 关联维度 | 主题相似、观点互补、同一领域不同角度 |
| 更新时机 | 新文章入库时自动计算与已有文章的关联 |
| 跨时间 | 不限于近期，能关联到数月前的旧收藏 |

---

### F12. 跨文章综合（远期 · P2 · Pro+）

**用户感受**：问 Folio"我收藏的文章里，关于降低 LLM 成本有哪些方案？"——AI 从 8 篇不同文章中提取、整合、对比，给出综合答案并标注每个观点来自哪篇文章。

不是单篇文章的问答，而是把整个收藏库当作知识库来查询。让收藏真正变成**可检索、可综合的个人知识引擎**。

| 项目 | 要求 |
|------|------|
| 输入 | 用户自然语言提问 |
| 检索范围 | 用户全部收藏文章 |
| 输出 | 综合多篇文章的结构化回答，标注每个观点的来源文章 |
| 来源可追溯 | 点击来源标注可跳转到对应文章的相关段落 |
| 对话能力 | 支持追问和澄清（"再详细说说第 2 点"） |

---

### F13. 智能专题（远期 · P2 · Pro+）

**用户感受**：收藏了十几篇关于 AI Agent 的文章后，Folio 自动生成了一个「AI Agent」专题页——有概览、有分主题整理、有时间线，一眼看清自己在这个领域的知识积累脉络。

当围绕某个主题积累了足够多的文章时，AI 自动创建并维护主题专题。

| 项目 | 要求 |
|------|------|
| 触发条件 | 同一主题积累 ≥ 5 篇文章时自动生成 |
| 专题内容 | 主题概览（AI 生成综述）+ 文章按子主题分组 + 时间线 |
| 自动更新 | 新收藏相关文章时自动归入已有专题，更新概览 |
| 用户可控 | 可手动合并/拆分/重命名专题 |
| 展示位置 | 收藏库中独立的「专题」入口 |
...
**用户感受**：收藏了一篇关于定价策略的好文章，3 周后 Folio 轻轻提醒："你之前收藏过这个洞察——还记得吗？"不是强制复习，而是在你快要忘记的时候，帮你想起来。

解决收藏工具最大的痛点——"存了就忘"。基于遗忘曲线，在合适的时间重新浮现关键洞察。

| 项目 | 要求 |
|------|------|
| 唤醒内容 | AI 提取的文章核心洞察（1-2 句话），不是完整摘要 |
| 唤醒频率 | 每天最多 1-2 条，不造成信息负担 |
| 唤醒时机 | 基于收藏时间递减频率（第 3 天、第 7 天、第 21 天、第 60 天） |
| 展示方式 | App 内卡片或推送通知，点击可展开原文 |
| 用户反馈 | "已掌握"（不再提醒）/"稍后再看"（重新排期） |

---

### F15. 时效预警（远期 · P3 · Pro+）

**用户感受**：收藏了一篇 React 18 的教程，一年后 React 已经到 19.x 了，Folio 标记提醒"这篇内容可能已过时"——帮你维护一个鲜活的知识库。

AI 检测收藏内容是否可能因时间推移而过时，特别针对技术教程、工具评测、版本相关内容。

| 项目 | 要求 |
|------|------|
| 检测范围 | 技术教程、工具评测、版本相关内容（非所有文章） |
| 检测信号 | 文章提及的版本号、框架名、API 等是否有重大更新 |
| 提醒方式 | 文章卡片/阅读页顶部显示"内容可能已过时"标记 |
| 用户操作 | "标记为仍然有效" / "查找更新版本" |
...
| **每日播客** | 不支持 | 不支持 | 支持（每日语音日报 + 文本摘要） |
| **知识关联** | 不支持 | 不支持 | 支持（自动发现文章间关联） |
| **跨文章综合问答** | 不支持 | 不支持 | 支持（基于收藏库综合回答） |
| **智能专题** | 不支持 | 不支持 | 支持（自动生成主题专题页） |
| **知识唤醒** | 不支持 | 不支持 | 支持（遗忘曲线定期浮现洞察） |
| **时效预警** | 不支持 | 不支持 | 支持（检测内容是否过时） |
| **iCloud 同步** | 不支持 | 不支持 | 支持 |
...
> 注：远期增值功能（F10-F15）的 AI 成本待各功能技术方案确定后补充。
