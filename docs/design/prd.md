...
AI 增值功能（三个魔法时刻）
├── F10. 洞察级摘要 — 升级摘要：提炼文章最值得记住的核心洞察
├── F11. 相关收藏   — 阅读页底部自动展示语义相关的已收藏文章
└── F12. 知识问答   — 基于个人收藏库的 RAG 问答，答案溯源到原文
```
...
| F10. 洞察级摘要 | P1 | 否 | 升级 AI 摘要：从客观概述到提炼核心洞察 |
| F11. 相关收藏 | P2 | 否 | 阅读页底部展示语义相关的已收藏文章 |
| F12. 知识问答 | P1 | 否 | 基于个人收藏库的 RAG 问答，答案溯源到原文 |
...
| 项目 | 要求 |
|------|------|
| 分类体系 | 单级分类（9 个预设分类，系统固定，用户不可增删） |
| 分类方式 | AI 自动判断，单篇文章归入 1 个分类 |
...
> 注：初始设计曾计划采用二级分类（10 个大类 + 子类），实际实现简化为 9 个单级分类，以降低 AI 分类复杂度并提高准确率。
...
| 摘要语言 | 与原文语言一致 |
| 摘要风格 | Free 版：客观概述；Pro/Pro+ 版：洞察级摘要（见 F10），提炼核心洞察和反直觉结论 |
...
| 用途 | 模型 | 理由 |
|------|------|------|
| 分类 + 标签 + 洞察摘要（单次调用） | DeepSeek Chat (deepseek-chat) | 性价比最优，速度快，质量足够 |
| 知识问答 RAG（Pro+） | DeepSeek Chat (deepseek-chat) | 综合多篇文章回答用户问题 |
| 向量嵌入 | 待定（text-embedding-3-small 或开源） | 语义搜索 + 相关收藏 + RAG 检索 |
...
| AI 语义搜索 | 基于向量嵌入的语义搜索，与关键词搜索融合排序，用户无感知切换（Pro+ 功能，复用 F12 嵌入基础设施） | P1 |
...
> **设计哲学**：AI 不是在"总结"文章，而是在回答"这篇文章最值得记住的是什么"。

**用户感受**：收藏一篇文章，几秒后看到的摘要不是无聊的内容概述，而是让你恍然大悟的核心洞察——"对，这就是我存它的原因"。

#### 当前问题

现有摘要是客观概述："本文讨论了大语言模型的训练方法论，包括预训练和微调阶段。"——正确但无用，用户读完摘要还是不知道这篇文章有什么独特价值。

#### 目标

摘要应该是洞察提炼："训练 LLM 的关键不在数据量，而在数据质量——1T 高质量 token 胜过 10T 噪声数据。"——用户读完摘要就能回忆起为什么收藏它。

#### 规格

| 项目 | 要求 |
|------|------|
| 摘要风格 | 提炼核心洞察/独特观点/反直觉结论，而非平铺直叙的概述 |
| 摘要结构 | 1 句核心洞察 + 2-3 个支撑论点 |
| key_points 升级 | 从"要点列表"变为"值得记住的洞察"，每条应能独立成立 |
| 语言 | 与原文一致 |
| 实现方式 | 优化 AI prompt，无需新增 API 或数据模型变更 |

#### 验收标准

- [ ] 对比新旧摘要，新摘要能让用户在不读原文的情况下获取核心价值
- [ ] 新摘要避免"本文介绍了…"、"作者讨论了…"等废话开头
- [ ] 在 50 篇测试文章上，新摘要的用户偏好率 > 70%

---

### F11. 相关收藏（P2 · Pro+）

> **设计哲学**：不做知识图谱可视化，不做独立页面。在你阅读时安静地说一句"你之前也存过类似的"。

**用户感受**：读一篇关于向量数据库的文章，滑到底部看到"你 2 周前收藏的《RAG 实践指南》与此相关"——原来这些知识是连在一起的。
...
| 展示位置 | 阅读页底部，正文结束后 |
| 展示形式 | 水平滚动卡片，最多 3 篇 |
| 关联依据 | 向量嵌入的语义相似度（复用 F12 的嵌入基础设施） |
| 更新时机 | 新文章入库生成嵌入时，自动计算与已有文章的相似度 |
| 跨时间 | 不限近期，能关联到数月前的旧收藏 |
| 用户交互 | 点击跳转到对应文章阅读页 |

#### 验收标准

- [ ] 相关文章确实主题相关（人工评估 > 80% 相关）
- [ ] 收藏不足 3 篇相关文章时优雅降级（不显示该区域）
- [ ] 不影响阅读页加载速度
...
> **设计哲学**：你的收藏库不是死的档案室，而是一个读过你所有文章的研究助手。答案来自你亲手挑选的内容，每一句都可以溯源到原文。

**用户感受**：存了 30 篇关于独立开发的文章，某天想做 side project，直接问"我存过的文章里，关于获取前 1000 个用户有哪些方法？"——Folio 综合多篇文章回答，并标注"根据你 3 月 5 日存的那篇 Indie Hacker 访谈……"

#### 与通用 AI 对话的区别

| | ChatGPT / DeepSeek | Folio 知识问答 |
|---|---|---|
| 数据来源 | 互联网训练数据 | 你亲手收藏的文章 |
| 答案可信度 | 可能幻觉 | 每句话溯源到原文段落 |
| 个性化 | 通用回答 | 基于你的阅读偏好和知识积累 |
| 时效性 | 训练数据截止日 | 你最新收藏的文章 |
...
问题向量化（Embedding）
    ↓
在用户收藏库的向量索引中检索 Top-K 相关段落
    ↓
将相关段落 + 用户问题注入 LLM Prompt
    ↓
LLM 生成综合回答 + 来源标注
    ↓
返回答案（每个观点附带来源文章链接）
...
| 项目 | 要求 |
|------|------|
| 入口 | 搜索页 — 当搜索无结果或用户主动切换到"问答"模式 |
| 展示形式 | 内联在搜索结果区域，不跳转新页面 |
| 来源标注 | 每个观点附带来源文章标题，点击可跳转到原文对应段落 |
| 对话能力 | 支持 1-2 轮追问（"再详细说说第 2 点"） |
| 检索范围 | 用户全部收藏文章 |
| 向量嵌入 | 文章入库时异步生成嵌入，存储在本地向量索引 |
| 嵌入模型 | 待定（OpenAI text-embedding-3-small 或开源替代） |
| 回答模型 | DeepSeek Chat |
| 响应时间 | < 5 秒 |
| 无结果处理 | "你的收藏中暂无相关内容" — 不回退到通用 AI 回答 |

#### 语义搜索（附带能力）

向量嵌入基础设施建好后，**语义搜索自动获得**——用户在搜索框输入"那个用咖啡渣做建材的创业公司"，即使原文用的是 "coffee grounds" 和 "construction material"，也能找到。

| 项目 | 要求 |
|------|------|
| 触发方式 | 搜索框输入 → 同时执行 FTS5 关键词搜索 + 向量语义搜索 |
| 结果合并 | 两种搜索结果融合排序，关键词精确匹配优先 |
| 用户感知 | 用户无需知道背后是什么技术，搜索框就是搜索框 |
...
| **AI 自动摘要** | 不支持 | 支持 | 支持 |
| **全文搜索** | 本地关键词搜索 | 本地关键词搜索 | 关键词 + AI 语义搜索 |
| **AI 洞察级摘要** | 不支持（基础概述） | 支持（核心洞察提炼） | 支持（核心洞察提炼） |
| **相关收藏** | 不支持 | 不支持 | 支持（阅读页底部展示） |
| **AI 知识问答** | 不支持 | 不支持 | 支持（基于收藏库 RAG 问答） |
| **iCloud 同步** | 不支持 | 不支持 | 支持 |
...
| Pro+ 知识问答（RAG） | 约 30 次/月 | DeepSeek Chat | ~$0.01/次 | ~$0.30 |
| Pro+ 向量嵌入 | 约 100 篇/月 | Embedding 模型 | ~$0.0004/篇 | ~$0.04 |

**Pro 用户月 AI 成本**：约 $0.05（¥0.35）
**Pro+ 用户月 AI 成本**：约 $0.44（¥3.2）
...
| 2026-03-02 | 1.2 | 新增远期增值功能 F10-F15 — 每日播客、知识关联、跨文章综合、智能专题、知识唤醒、时效预警（均为 Pro+ 远期功能） |
| 2026-03-07 | 2.0 | **AI 功能重新设计**（Jobs 式减法）— 砍掉 F13 智能专题、F14 知识唤醒、F15 时效预警；F10 改为洞察级摘要（升级 prompt）；F11 简化为阅读页底部相关收藏；F12 改为基于 RAG 的知识问答；F5 搜索集成语义搜索。核心理念：三个魔法时刻——存得更聪明、找得更准、用得更深 |
